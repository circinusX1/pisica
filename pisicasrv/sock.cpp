...
//-----------------------------------------------------------------------------
SOCKET sock::create(const SADDR_46&, int)
{
    return (SOCKET)-1;
}
...
//-----------------------------------------------------------------------------
int sock::get_option(int options)
{
    if(isopen())
    {
        int optionVal;
        socklen_t optLen = sizeof(optionVal);
        if(0 == getsockopt(_thesock, SOL_SOCKET, options, (char*)&optionVal, &optLen))
            return optionVal;
    }
    return -1;
}
...
int sock::set_option(int options, int optionval)
{
    if(isopen())
        return  setsockopt (_thesock, SOL_SOCKET, options, &optionval, sizeof (optionval));
    return -1;
}
...
SOCKET tcp_srv_sock::create(int port, int opt, const char* iface)
{
    ...
    // options before bind will allow recreation on same SIN when closed imediatelly
    if(opt)
    {
        int one = 1;
        ::setsockopt(_thesock, SOL_SOCKET, /*SO_REUSEADDR*/opt, (char *)&one, sizeof(one));
...
SOCKET tcp_srv_sock::accept(tcp_cli_sock& cliSock)
{
    ...
    {
        //prinf("accept " << errno <<", " << strerror(errno));
        _error = errno;
        cliSock.destroy();
        return 0;
    }
...
SOCKET tcp_cli_sock::create(const SADDR_46&, int opt)
{
