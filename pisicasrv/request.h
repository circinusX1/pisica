...
struct urlreq {
    ...
    std::string inspect() const
    {
        ...
        for(std::vector<urlreq::HeaderItem>::const_iterator it = headers.begin();
            it != headers.end(); ++it)
        {
...
public:
    ...
    ParseResult parse(urlreq &req, const char *begin, const char *end)
    {
        return consume(req, begin, end);
    }

private:
    static bool checkIfConnection(const urlreq::HeaderItem &item)
    {
        return strcasecmp(item.name.c_str(), "Connection") == 0;
    }

    ParseResult consume(urlreq &req, const char *begin, const char *end)
    {
        while( begin != end )
        {
            char input = *begin++;
            ...
            case HeaderLineStart:
                ...
                {
                    req.headers.push_back(urlreq::HeaderItem());
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    ...
                    {
                        urlreq::HeaderItem &h = req.headers.back();
            ...
            case ExpectingNewline_3: {
                std::vector<urlreq::HeaderItem>::iterator it = std::find_if(req.headers.begin(),
                                                                             req.headers.end(),
                                                                             checkIfConnection);
